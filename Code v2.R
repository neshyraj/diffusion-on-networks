...
#Calculate Mixing Matrix raw values (Code adapted from GitHub not entirely right)
mixmat <- function(mygraph, attrib, use.density=FALSE) {
...
#To correct mixing matrix for 2x2 matrices
...
#Louvain modularity clustering. Gets stuck cause louvain function is slow idk.
...
#Just run Louvain with default resolution. Probably don't actually need resolution.
#clusts <- list() so i dont fucking accidentally run this
for (i in 1:43){
...
with(assort_adopt,plot(a_cas,V9,col = "blue",xlab = "Caste Community Level", ylab = "Adoption Rate",main = "Adoption Rate by Caste Community"))
clip(0.15,0.75,0,1)
abline(reg_cas)
...
#Combine all leaders and query characteristics to train logistic regression model on
all_leaders <- NULL
for (i in 1:43){
